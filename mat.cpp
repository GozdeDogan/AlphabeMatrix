 ...
 * Author: GOZDEDOGAN
 * 
 * Created on January 18, 2020, 7:24 PM
 */
...
//
//  Note: although it uses string it uses printf because I hate cout's
//  awkwardness. :-)
...
Matrix *MatrixRowIter::rowBegin()
{
    ...
    more = true;
    arow->defined = true;
    //cout << "Matrix *MatrixRowIter::rowBegin()" << "\tarow->defined = true" << endl;
...
Matrix *MatrixRowIter::rowNext()
{
    if (r < mat->maxr-1) {
        ...
        for (int i=0; i<mat->maxc; i++) arow->m[0][i] = mat->m[r][i];
        arow->defined = true;
        //cout << "Matrix *MatrixRowIter::rowNext()" << "\tarow->defined = true" << endl;
        arow->submatrix = false;
    }
    else {
        more = false;
...
void Matrix::allocate(int r, int c, string namex, bool isSubMatrix) 
{
    if (isSubMatrix) c=-1;
    maxr = r;
    maxc = c;
    ...
    defined = false;
    //cout << "void Matrix::allocate(int r, int c, string namex, bool isSubMatrix)" << "\tdefined = false" << endl;
...
bool Matrix::deallocate()
{
    ...
    /*cout << "m matrix in deallocate start >> " << endl;
    ...
    if (debug) 
        printf("DEBUG(deallocate): name \"%s\", size %d X %d\n", name.c_str(), maxr, maxc);

    defined = false;
    //cout << "bool Matrix::deallocate()" << "\tdefined = false" << endl;
    submatrix = false;
...
void Matrix::reallocate(int otherMaxr, int otherMaxc, string namex)
{
...
Matrix::Matrix(int r, string namex)
{
    allocate(r, -1, namex, true);       // WARNING: allocate as subMatrix!!!
}


Matrix::Matrix(int r, int c, string namex)
{
    allocate(r, c, namex);
}


Matrix::Matrix(int r, int c, double value, string namex)
{
    allocate(r, c, namex);
    constant(value);
}
...
Matrix::Matrix(int r, int c, double *data, string namex)
{
    allocate(r, c, namex);
    ...
    defined = true;
    //cout << "Matrix::Matrix(int r, int c, double *data, string namex)" << "\tdefined = true" << endl;
}


// copy constructor
Matrix::Matrix(const Matrix &other, string namex)
{
    ...
    defined = true;
    //cout << "Matrix::Matrix(const Matrix &other, string namex)" << "\tdefined = true" << endl;
}
...
Matrix::Matrix(Matrix *other)
{
    ...
    defined = true;
    //cout << "Matrix::Matrix(Matrix *other)\t" << "defined = true" << endl;
}
...
const string &Matrix::getName(const string &defaultName) const
{
...
{
    defined = true;
    //cout << "void Matrix::setDefined()\t" << "defined = true" << endl;
}
...
// do bounds checking
void Matrix::checkBounds(int r, int c, string msg) const
{
...
void Matrix::assertDefined(string msg) const
{
   // cout << "\nassertDefined " << msg << " ";
    if (!defined)
        cout << "FALSE" << endl;
...
void Matrix::assertUsableSize(string msg) const
{
    if (maxr < 0 || maxc < 0) {
        if (name.length()==0)
...
void Matrix::assertSquare(string msg) const
{
    assertDefined(msg);
...
// assert size is rxc
void Matrix::assertSize(int r, int c, string msg) const
{
    assertDefined(msg);
    if (maxr != r || maxc != c) {
        if (name.length()==0)
...
void Matrix::assertIndexOK(int r, int c, string msg) const
{
    if (r<0 || r>=maxr || c<0 || c>=maxc) {
        if (name.length()==0) {
...
void Matrix::assertOtherLhs(const Matrix &other, string msg) const
{
    if (maxc!=other.maxr) {
...
// assert other can be lhs of mult op
void Matrix::assertRowsEqual(const Matrix &other, string msg) const
{
    if (maxr!=other.maxr) {
...
// assert other can be lhs of mult op
void Matrix::assertColsEqual(const Matrix &other, string msg) const
{
    if (maxc!=other.maxc) {
...
void Matrix::assertOtherSizeMatch(const Matrix &other, string msg) const
{
...
void Matrix::assertRowVector(string msg) const
{
    if (maxr!=1) {
        if (name.length()==0) {
...
void Matrix::assertColVector(string msg) const
{
    if (maxc!=1) {
        if (name.length()==0) {
...
Matrix &Matrix::operator=(const Matrix &other)
{
    ...
    defined = true;
    //cout << "Matrix &Matrix::operator=(const Matrix &other)\t" << "defined = true" << endl;
...
Matrix Matrix::extract(int minr, int minc, int sizer, int sizec)
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::extract(int minr, int minc, int sizer, int sizec)\t" << "out.defined = true" << endl;
...
Matrix Matrix::extractStride(int minr, int minc, int stepr, int stepc)
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::extractStride(int minr, int minc, int stepr, int stepc)\t" << "out.defined = true" << endl;
...
Matrix &Matrix::extract(int minr, int minc, int sizer, int sizec, Matrix &out)
{
    ...
    out.defined = true;
    //cout << "Matrix &Matrix::extract(int minr, int minc, int sizer, int sizec, Matrix &out)\t" << "out.defined = true" << endl;
...
Matrix Matrix::argMinRow() const
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::argMinRow() const\t" << "out.defined = true" << endl;
...
Matrix Matrix::minRow() const
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::minRow() const\t" << "out.defined = true" << endl;
...
Matrix Matrix::normalizeCols()
{
    ...
    minMax.defined = true;
   // cout << "Matrix Matrix::normalizeCols()\t" << "minMax.defined = true" << endl;
...
double Matrix::dist2(const Matrix &other) const
{
    ...
    /*cout << "recovered data in dist2>>" << endl;
...
// matrix multiply (matrix carpimi)
...
Matrix Matrix::pickRows(int match, const Matrix &list, int &num)
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::pickRows(int match, const Matrix &list, int &num)\t" << "out.defined = true" << endl;
    return out;
}
...
Matrix Matrix::dot(const Matrix &other)
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::dot(const Matrix &other)\t" << "out.defined = true" << endl;
    return out;
}
...
Matrix Matrix::dotT(const Matrix &other)
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::dotT(const Matrix &other)\t" << "out.defined = true" << endl;
...
Matrix Matrix::Tdot(const Matrix &other)
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::Tdot(const Matrix &other)\t" << "out.defined = true" << endl;
    return out;
}
...
Matrix Matrix::meanVec()
{
    ...
    mean.defined = true;
    //cout << "Matrix Matrix::meanVec()\t" << "mean.defined = true" << endl;
...
Matrix Matrix::stddevVec() 
{
    ...
    stddev.defined = true;
    //cout << "Matrix Matrix::stddevVec()\t" << "stddev.defined = true" << endl;
...
Matrix Matrix::cov()
{
    ...
    out.defined = true;
   // cout << "Matrix Matrix::cov()\t" << "out.defined = true" << endl;
    delete [] mean;
...
//            out.m[r][c] = out.m[c][r] = sum * inv;
    ...
    out.defined = true;
   // cout << "Matrix Matrix::cov(Matrix &other)\t" << "out.defined = true" << endl;
    delete [] mean;
    delete [] meano;
...
Matrix Matrix::cartesianRow(double (*f)(int size, double *x, double *y), Matrix &other)
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::cartesianRow(double (*f)(int size, double *x, double *y), Matrix &other)\t" << "out.defined = true" << endl;
    return out;
}
...
Matrix &Matrix::mapIndex(double (*f)(int r, int c, double x))
{
    ...
    //cout << "Matrix &Matrix::mapIndex(double (*f)(int r, int c, double x))\t" << "defined = true" << endl;
    return *this;
}
...
Matrix &Matrix::constant(double x)
{
    ...
    defined = true;
    //cout << "Matrix &Matrix::constant(double x)\t" << "defined = true" << endl;
    return *this;
}
...
Matrix &Matrix::constantCol(int c, double x)
{
    ...
    defined = true;
   // cout << "Matrix &Matrix::constantCol(int c, double x)\t" << "defined = true" << endl;
    return *this;
}
...
Matrix &Matrix::constantColRange(int c, double start, double step)
{
    ...
    defined = true;
    //cout << "Matrix &Matrix::constantColRange(int c, double start, double step)\t" << "defined = true" << endl;
    return *this;
}
...
Matrix &Matrix::constantDiagonal(double x)
{
    ...
    defined = true;
   // cout << "Matrix &Matrix::constantDiagonal(double x)\t" << "defined = true" << endl;
    return *this;
}
...
Matrix &Matrix::rand(double min, double max)
{
    ...
    defined = true;
    //cout << "Matrix &Matrix::rand(double min, double max)\t" << "defined = true" << endl;
    return *this;
}
...
Matrix &Matrix::rand(int min, int max)
{
    ...
    defined = true;
   // cout << "Matrix &Matrix::rand(int min, int max)\t" << "defined = true" << endl;
    return *this;
}
...
Matrix &Matrix::sample(Matrix &out)
{
    ...
    out.defined = true;
    //cout << "Matrix &Matrix::sample(Matrix &out)\t" << "out.defined = true" << endl;
    return out;
}
...
Matrix Matrix::transpose()
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::transpose()\t" << "out.defined = true" << endl;
    return out;
}
...
Matrix &Matrix::transposeSelf()
{
    ...
    // handle non-square matrix with reallocation
        ...
        m = newm;
        defined = true;
        //cout << "Matrix &Matrix::transposeSelf()\t" << "defined = true" << endl;
    }
...
void Matrix::printSize(string msg) const
{
    if (msg.length()) {
        printf("%s ", msg.c_str());
    }
...
void Matrix::print(string msg) const
{
    assertDefined("print");
...
// print the whole matrix including it's name and size
void Matrix::printInt(string msg) const
{
...
void Matrix::read()
{
    ...
    defined = true;
    //cout << "void Matrix::read()\t" << "defined = true" << endl;
}
...
Matrix Matrix::eigenSystem()
{
    ...
    //cout << "Matrix Matrix::eigenSystem()\t" << "values.defined = true" << endl;
    return values;
}
...
Matrix Matrix::subMatrix(int minr, int minc, int sizer, int sizec) const
{
    ...
    out.defined = true;
    //cout << "Matrix Matrix::subMatrix(int minr, int minc, int sizer, int sizec) const\t" << "out.defined = true" << endl;
    return out;
}    
...
Matrix Matrix::subMatrixEq(int c, double value) const
{
    ...
    vector<double *> rowList;        // this is a retrofit of using an array originally when vector better
    ...
    out.defined = true;
    //cout << "Matrix Matrix::subMatrixEq(int c, double value) const\t" << "out.defined = true" << endl;
    return out;
}    
...
Matrix Matrix::subMatrixNeq(int c, double value) const
{
    ...
    out.defined = true;
   // cout << "Matrix Matrix::subMatrixNeq(int c, double value) const\t" << "out.defined = true" << endl;
...
Matrix Matrix::readAudio(string filename, string namex)
{
    WaveFunction* a=new WaveFunction(128,13);
      
    vector<vector<float> > mfccs1 = a->getMFCCs(filename);

    int row = mfccs1.size(), col = mfccs1[0].size();
    ...
    cout << "\n FINISH_VECTOR\n" << endl;*/
    // reallocate myself
    reallocate(row, col, name);
    
    for (int i = 0; i < row; i++) { 
        for (int j = 0; j < col; j++) 
            this->set(i, j, mfccs1[i][j]);  
    }
...
Matrix Matrix::readAudioWav(string filename, string namex)
{
    return readAudio(filename, namex);  
}
...
// in the filename
void Matrix::writeAudioWav(string filename, string comment)
{
    FILE *OUT;

    assertDefined("writeAudioWav");
    if (maxc%3 != 0) {
        if (name.length()==0) {
            printf("ERROR(writeAudioWav): Number of columns %d not divisible by three but supposed to be a matrix of RGB values.\n", maxc);
        }
        else {
            printf("ERROR(writeAudioWav): Number of columns %d in matrix named \"%s\" is not divisible by three but supposed to be a matrix of RGB values .\n", maxc, name.c_str());
        }
        exit(1);
    }
    ...
    if (filename.length()>0) {
        ...
        if (OUT==NULL) {
            printf("ERROR(writeAudioWav): Trying to open file \"%s\" but failed.\n", filename.c_str());
            exit(1);
        }
    }
    else {
