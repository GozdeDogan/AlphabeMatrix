 ...
 * Author: GOZDEDOGAN
 * 
 * Created on January 18, 2020, 7:22 PM
 */
...
vector<vector<float> > WaveFunction::getMFCCs(string filename){
    ...
    short buffer[FrmLen];
    float data[FrmLen];
    float FiltCoe1[FFTLen/2+1];  //sol katsayisi
    float FiltCoe2[FFTLen/2+1];  //dogru katsayisi
    int Num[FFTLen/2+1];     //Her bir noktanin hangi filtreye ait olduguna karar verilir
    float En[FiltNum+1];         //Bant enerjisi
    vector<complex<float> > vecList;

    sourcefile=fopen(filename.c_str(),"rb");
    fread(&waveheader,sizeof(mTWavHeader),1,sourcefile);
    InitHamming();//Hamming penceresi baslatilir
    InitFilt(FiltCoe1,FiltCoe2,Num); //MEL filtre kat sayilari baslatilir
    ...
    while(fread(buffer,sizeof(short),FrmLen,sourcefile)==FrmLen){
        ...
        MFCC(En);
        vecList.clear();
        fseek(sourcefile, -FrmLen/2, SEEK_CUR);//Cerceve kaymasi goz onune alinarak her cerceve hareket ettirilir
    }
...
void WaveFunction::FFT(const unsigned long &ulN, vector<complex<float> > &vecList){
    ...
    unsigned long ulPower = 0; //bir guc
    ...
    bitset<sizeof(unsigned long) * 8> bsIndex; //ikili konteyner
    unsigned long ulIndex; //ters seri numarasi
    unsigned long ulK;
    ...
    unsigned long ulGroupLength = 1; //segment uzunlugu
    unsigned long ulHalfLength = 0; //segment uunlugunun yarisi
    unsigned long ulGroupCount = 0; //segment sayisi
...
float WaveFunction::ComputeDTW(float *cep1, float *cep2, int num1, int num2){
    ...
    {	
        ...
        float minvalue;
        int stepnum;
        bool recheck;               //noktanin kaydedilmis olup olmadigi kontrol degiskeni
    };
    record * re;
    record * newre;

    newre=new record[num1*num2];    //Bir sonraki katmandaki tum noktalar kaydedilir
    re=new record[num1*num2];       //Gecerli katmandaki tum noktalar kaydedilir
    int renum;
    int newrenum=0;
    int i,j;
    point * poi;
    ...
    for(i=0;i<num1*num2;i++)
    {
        ...
        poi[i].minvalue=INF;
        poi[i].stepnum=0;
    }	//Baslangic degerleri ayarlandi

    for(i=0;i<5;i++)                //baslangic noktasi
    {
        if(i==0)  
        {	
            re[i].x=1; 
